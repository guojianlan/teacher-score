...
export default function StudentsPage() {
  ...
  return (
    <Stack gap={8}>
      ...
      <Box>
        {loading ? (
          <Flex p={12} justify="center"><Spinner color="fg.subtle" /></Flex>
        ) : students.length === 0 ? (
          <Box py={16} textAlign="center" borderTop="1px solid" borderBottom="1px solid" borderColor="border.default">
            ...
            <Text color="fg.subtle">还没有学生。点右上角「新建学生」开始。</Text>
          </Box>
        ) : (
          <Table size="md">
            <Thead>
            ...
            <Tbody>
              {students.map((s) => (
                <Tr key={s.id} _hover={{ bg: 'bg.surfaceSubtle' }} transition="background 100ms ease">
                  <Td fontWeight={500} fontSize="md">{s.name}</Td>
                  <Td color="fg.subtle">{s.grade ?? '—'}</Td>
                  <Td>
                    <Flex gap={1.5} wrap="wrap">
                      {(s.subjects ?? []).map((sub) => (
                        <Badge key={sub} bg="bg.muted" color="fg.muted">{SUBJECT_LABELS[sub] ?? sub}</Badge>
                      ))}
                    </Flex>
                  </Td>
                  ...
                  <Td>
                    ...
                    <Button size="xs" variant="ghost" color="status.danger.fg" onClick={() => onDelete(s.id)}>
                      删除
                    </Button>
                  </Td>
                </Tr>
