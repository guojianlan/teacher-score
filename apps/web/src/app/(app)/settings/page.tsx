...
export default function SettingsPage() {
  ...
  return (
    <Stack spacing={10}>
        ...
        <form onSubmit={changePassword}>
          <Stack spacing={4} maxW="sm">
            <FormControl isRequired>
              <FormLabel fontSize="sm" color="ink.700" mb={1.5}>当前密码</FormLabel>
              <Input type="password" value={oldPassword} onChange={(e) => setOldPassword(e.target.value)} autoComplete="current-password" />
            </FormControl>
            <FormControl isRequired>
              <FormLabel fontSize="sm" color="ink.700" mb={1.5}>新密码</FormLabel>
      ...
      <Section label="数据导出">
        <Text color="ink.500" fontSize="sm" mb={4} lineHeight={1.6}>
      ...
      <Section label="危险操作" danger>
        ...
        <Button variant="outline" color="danger.500" borderColor="danger.500"
          _hover={{ bg: 'danger.100', borderColor: 'danger.500' }}
...
function Section({ label, danger, children }: { label: string; danger?: boolean; children: React.ReactNode }) {
  return (
    <Box pb={8} borderBottom="1px solid" borderColor="ink.100">
      <Text fontFamily="mono" fontSize="xs" color={danger ? 'danger.500' : 'ink.500'} letterSpacing="0.08em" textTransform="uppercase" mb={4}>
        {label}
      </Text>
      {children}
    </Box>
